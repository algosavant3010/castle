...
export function Topbar() {
  return (
    <header className="sticky top-0 z-50 flex h-12 md:h-14 items-center justify-between border-b border-white/[0.04] bg-transparent px-4 md:px-6 overflow-visible">
      {/* Logo */}
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 flex items-center justify-around border-t border-white/[0.04] bg-[#08080a]/95 backdrop-blur-xl py-1.5 px-1 safe-bottom md:hidden">
        {[
          ...
          <Link
            key={link.href}
            href={link.href}
            className="flex flex-col items-center gap-0.5 px-1.5 py-1 rounded-lg transition-colors text-faint active:text-accent"
          >
